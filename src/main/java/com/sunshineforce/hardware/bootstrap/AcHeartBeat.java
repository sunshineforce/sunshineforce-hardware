...
public class AcHeartBeat {
    ...
    public static void requestSelect(Wifi wifi) {
        while(true){
    ...
    public static void responseHeartBeatWithAcConfig(Wifi wifi) {
        while(true){
            DatagramSocket socket = null;
            try {
                byte[] heartBeatBuf = new byte[15];
                ...
                if(REQUESTTYPE.equals(ByteUtil.ByteToHex(type).toLowerCase()) && wifi.getProbeMac().toLowerCase().equals(ByteUtil.ByteToHex(mac).toLowerCase())){
                    ...
                    DatagramPacket configPacket = new DatagramPacket(configMsg, configMsg.length, heartBeatResponsePacket.getSocketAddress());
    ...
    public static void responseHeartBeatWithToggleWifi(Wifi wifi) {
        while(true){
            DatagramSocket socket = null;
            try {
                byte[] heartBeatBuf = new byte[15];
    ...
    public static void responseHeartBeatWithSetServer(Wifi wifi) {
        while(true){
            DatagramSocket socket = null;
            try {
                byte[] heartBeatBuf = new byte[15];
